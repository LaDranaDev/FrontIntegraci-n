...
@Component({
  selector: 'app-detalles-operaciones',
  templateUrl: './detalles-operaciones.component.html',
  styleUrls: ['./detalles-operaciones.component.css']
})
export class DetallesOperacionesComponent implements OnInit {
  operaciones: any
  bloqueado = true
  ...
  reintentarD() {
    ...
    this.reintentar(this.datosForm.value.idEstatus, this.operaciones);
  }
  rechazarD() {
  ...
  reintentar(estatus: any, data: any) {
    ...
    const msg = this.dialog.open(ModalInfoComponent, {
      data: new ModalInfoBeanComponents(
        this.translate.instant('consultaadmonusuario.msjINF00001Titulo'),
        this.translate.instant('MO.cambioEstatus'),
        'confirm',
        '',
        ''
      ), hasBackdrop: true
        ...
        if (this.horario) {
          await this.monitor.cambiarEstatusOperacion(data).then(
            async (resp: any) => {
              ...
              this.open(this.translate.instant('consultaadmonusuario.msjINF00001Titulo'), this.translate.instant('pantalla.monitor.Inf02.mensaje'), 'info', this.translate.instant('pantalla.monitor.Inf02.codigo'))
